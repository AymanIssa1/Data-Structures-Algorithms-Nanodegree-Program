...
def create_graph(num_islands, bridge_config):
    ...
    print("bridge_config = ", bridge_config)
    print("graph = ", graph)
    return graph
...
def minimum_cost(graph):
    ...
    while len(minHeap) > 0:
        ...
        total_cost += cost
        print( "current_vertex = ", current_vertex, "cost = ", cost, "total_cost = ", total_cost,)

        for neighbour, edge_cost in graph[current_vertex]:
            heapq.heappush(minHeap, (edge_cost, neighbour))
